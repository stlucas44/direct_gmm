...
with open('bunny_1k_com_verts_tuesday_25.log','w') as fout:
    for km in [6,12,25,50,100,200,400]:
        print("Evaluation for k mixtures: ", km)
        for init in ['kmeans','random']:
            for exp_n in range(10):
                print("Evaluation for k mixtures: ", km)

                new_tol = 1e-3 #initially 1e-12
                gm3 = GaussianMixture(km,init_params=init,max_iter=25,tol=new_tol); gm3.set_covars(data_covar); gm3.set_areas(a); gm3.fit(com); gm3.set_covars(None);  gm3.set_areas(None)
                gm0 = GaussianMixture(km,init_params=init,max_iter=25,tol=new_tol); gm0.set_areas(a); gm0.fit(com); gm0.set_areas(None)
                gm1 = GaussianMixture(km,init_params=init,max_iter=25,tol=new_tol); gm1.fit(com)
                gm2 = GaussianMixture(km,init_params=init,max_iter=25,tol=new_tol); gm2.fit(mesh0.vertices)
